...
class MaskPointHead_fixfp16(nn.Module):
    ...
    def get_roi_rel_points_train(self, mask_pred, labels, cfg):
        ...
        point_coords = torch.rand(
            batch_size, num_sampled, 2, device=mask_pred.device, dtype=mask_pred.dtype)
        ...
        if num_random_points > 0:
            rand_roi_coords = torch.rand(
                batch_size, num_random_points, 2, device=mask_pred.device, dtype=mask_pred.dtype)
